<?php
	...
	$lista = extras::htmlentities(cat::getList(array(
		'columns' => [
...
?>
	<div class="mg-sec container">
		<div class="gd-100">
			<div class="pd-zero gd-60 gd-m-100"><h1><span class="icon-file-text"></span> Listado de Categorías</h1></div>
			<div class="pd-zero gd-40 gd-m-100 tx-right">
				<a href="<?php echo APP_ROOT; ?>/cats/create"><button type="button" class="btn btn-primary"><span class="icon-plus"></span></button></a>
			</div>
		</div>
		<div class="listable">
			<div class="container">
			...
			<?php foreach( $lista as $c ): ?>
				<div class="container">
					...
					<div class="gd-33 gd-m-50 tx-right">
						<a href="cats/update/<?php echo $c['id']; ?>"><button class="btn btn-primary size-s"><span class="icon-pencil"></span></button></a>
					</div>
				</div>
			<?php endforeach; ?>
			<?php if (!$lista): ?>
				<div class="alert">
				    <strong>¡Sin categorías!</strong> No se encontrarón resultados.
				</div>
			<?php endif; ?>
		</div>
	</div>
